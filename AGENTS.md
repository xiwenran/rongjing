...
- [x] 拼图图片文件夹多子文件夹导入时显示来源列表，可像多 PPT 一样逐份切换并分别保留设置
...
- [x] 拼图导入图片文件夹时跳过本软件已导出的拼图结果目录，避免把成品子文件夹再次当作输入来源
- [x] 拼图同名 PPT 缓存按路径哈希隔离；「导出全部」点击时冻结每份来源的配置快照
- [x] 拼图缩略图支持拖动排序，导出按调整后的页面顺序生成
...
29. **拼图输出目录不覆盖**：拼图页每次按来源名称分配新目录；同名时追加 `_2`、`_3`，旧目录不覆盖、不清理。
...
35. **拼图重复导出**：同一来源重复导出时先重算自动拆分张数，再分配新的来源目录；manifest 只记录本轮新目录内的成品，不再清理旧成品。
...
39. **拼图导入排除已导出目录**：拼图导出目录带 `.rongjing_collage_manifest.json`，再次导入父级图片文件夹时必须跳过这类目录及其子目录；否则已生成的 `拼图_*.jpg/png` 会被误当作新的输入图片或批量来源，造成预览内容和导出张数混乱。
40. **拼图拖动排序状态同步**：缩略图拖动排序必须同步 `self._image_files` 和当前多来源项里的文件顺序；排除状态要跟随原图片移动，来源状态 key 不能依赖当前第一张图片，否则拖动后会丢失该来源设置。
41. **统一过滤时机**：隐藏文件、AppleDouble `._*`、Office 临时文件 `~$*` 和非文件项必须在排序、计数、封面选择、manifest 生成和任务清单组装之前过滤，避免同一输入在不同入口得到不同页数或首图。
42. **输出目录永不复用**：拼图、图片合成、真实视频和资料导出每次运行都分配新来源目录；同名时追加 `_2`、`_3`，旧目录和 manifest 不覆盖、不清理。根目录直接图片使用所选文件夹名。
43. **视频入口按输入分流**：真实视频继续使用 `VideoRunner`；图片或文件夹走页面序列，单图静态，多图在 Mac 上优先使用 Core Image 曲面卷页，非 Mac 或助手不可用时回退 CPU 平面翻页，并使用固定 FPS、递增 PTS 和流式编码。
...
50. **音乐库与页面视频配乐**：音乐库固定在 App Data 的 `融景/music/`，视频导入仅保存首条音轨，所有导入按 SHA-256 去重。页面视频支持不配乐、固定和随机，默认音量 35%，从音频第 0 秒开始，短音频每轮同曲从第 0 秒循环并编码为 48 kHz 双声道 AAC；随机实际曲目写入完成回执。真实视频保持原声；翻页预览视频含 BGM，但弹窗当前不播放声音。M1/M2/M3 见 `7d3ebc2`、`00103fe`、`96ccba6`，独立审查 PASS WITH RISKS；AAC 尾垫与未持久 sidecar 为非阻断风险，真机听感、长音频和冻结 App 未验证。
51. **页面视频清晰度与零秒配乐**：正式页面视频复用批量图片的 `0`/1920/2560/3840 分辨率规则，预览固定 960，真实视频不套用；背景只在尺寸变化时用 LANCZOS 缩放。VideoToolbox 码率限制为 8–20 Mbps，libx264 使用 CRF 17。BGM 从源音频第 0 秒开始，每轮循环也回到第 0 秒。修复见 `9a6889d`；短样本 1920×1440 / 1024×768，编码前后 ROI 边缘能量 9.710 / 9.887，前 200 ms RMS 0.0963、源相关 0.9998；真机文字观感和冻结 App 未验证。
