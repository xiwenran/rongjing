...
- [x] 模板配置页按「教师场景 / 台式机电脑 / 笔记本室内 / 文档纸张 / 自定义场景」管理分类；合成类型不再展示，按分类自动推断
...
17. **模板分类与合成类型分离**：`category` 负责用户可见分组；UI 不展示 `template_type`，保存时按分类自动推断，`文档纸张` → `document_paper`，其他分类 → `screen`
