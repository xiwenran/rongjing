...
- 翻页预览弹窗、方向设置与 24 小时缓存清理 🚧
  - 当前阶段：S1 弹窗播放器 + 方向；S2 缓存清理 / 验证 / 收口。
  - 一句话现状：待将翻页预览改为固定弹窗内播放，默认右→左、可选左→右并保证正式导出与预览一致；预览缓存使用独立目录，软件启动时仅清理超过 24 小时的缓存且不碰正式成品。
  - 阻塞：无。
  - 最近验证：QtMultimedia 导入、弹窗控件/方向传参、CPU 左右镜像与缓存安全计划的定向检查通过；Swift helper 编译通过，主会话在沙箱外用 `left_to_right`、`progress=0.5` 成功输出 640×360 中间帧并目检确认左侧卷边、白色背面与阴影。真机视频解码与冻结 App 仍待用户验证。
  - commit hash：功能已实现，待独立审查后关闭专项并补最终 hash。
