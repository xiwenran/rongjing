...
- 图片批量合成动态内存降速 ✅ `6eef462` — 验证: 撤销 4K 固定 1 路限速，正常内存恢复原有最多 6 路；macOS 可用内存 ≤20% 临时降为 3 路、≤10% 降为 1 路，恢复至 ≥30% 自动回到 6 路，检测暂时失败时保持当前档位；4K 实拍任务先以 2 路起步再动态提升，界面显示实际降速/恢复状态。调度边界断言、批量合成回归及语法检查通过；1344 张真实外置盘批次与冻结 App 由用户验证
