...
- `--fit`：源图与模板承载区宽高比不一致时的适配方式，默认 `stretch`（拉伸铺满，零变化）；`contain` 等比缩放并补白（letterbox）到承载区比例，内容完整但纸面留边；`cover` 等比铺满并居中裁掉溢出，内容满版直达纸边不留补白——笔记场景默认推荐 `cover`（封面/满版页），密排正文页顶底会被裁、内容必须完整时用 `contain`
