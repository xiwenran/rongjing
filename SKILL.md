---
name: rongjing
description: 融景图片与资料处理：把图片嵌入实拍模板、生成带配乐的页面序列视频，或导出 PPT/Word 页面。触发词：融景、合成图、嵌入大屏、PPT嵌入背景、把图片嵌入模板、大屏合成、用模板合成、资料导出、页面翻页视频、翻页预览、Core Image翻页、音乐库、页面视频配乐、BGM。
---
...
- 页面视频需要配乐时，先在音乐库导入音频、文件夹或视频，再选择不配乐、固定或随机；视频导入只提取首条音轨，不保存画面。
...
- 音乐库固定使用 App Data 的 `融景/music/` 并按 SHA-256 去重。页面视频默认音量 35%，从音频第 0 秒开始，短音频每轮也从第 0 秒循环同一首曲目，输出 48 kHz 双声道 AAC；随机模式必须在完成回执中给出实际曲目。真实视频保持原声，不套用页面视频 BGM。
- 正式页面视频使用与批量图片相同的分辨率选择：`0` 为原始/模板尺寸，默认宽度 1920，可选 2560 或 3840；预览固定宽度 960，真实视频不套用。背景缩放使用 LANCZOS，同尺寸不重复 resize；VideoToolbox 使用 8–20 Mbps，libx264 使用 CRF 17。
- 翻页预览成品包含所选 BGM，但当前应用内预览弹窗仅显示画面；不能据此判断最终音频听感。
