...
- [x] macOS 页面图片文件夹入口改用 Qt 非原生多目录选择器，取消不改列表，选择器或导入异常时在应用内提示而不退出
...
44. **页面图片文件夹选择器不要依赖阻塞式 osascript**：冻结 App 在 macOS 原生多文件夹选择器完成后可能直接退出；该入口固定使用 Qt 非原生 `QFileDialog`，通过内部 list/tree view 的扩展选择模式支持多目录，并在选择与导入边界捕获异常后显示提示。其他单文件、单文件夹入口保持原逻辑。
