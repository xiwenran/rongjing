...
46. **表格复用应用级 Fusion style，不创建局部 QStyle**：`main.py` 已为 QApplication 设置 Fusion，`_set_green_selection` 只需设置表格和 viewport 的选中色 palette。移除局部 `QStyleFactory.create` / `setStyle` 是独立的样式生命周期优化；后续原生栈已证明本次页面文件夹选取后的崩溃发生在 macOS AX 查询 `accessibilitySelectedChildren` 时，不能再把本次根因归到局部 style 或 Python wrapper GC。
47. **macOS 视频表保持单 cell 选择，避免 AX 多 cell 查询崩溃**：真实原生 picker 在 `_save_dir`、`_populate_video_table` 和各 sync 全部返回后，`libqcocoa.dylib` 稳定于 `QMacAccessibilityElement::accessibilitySelectedChildren` 的 `selectedItems()` 循环触发 `exit 139`。Qt 6.10.2 的 `QAccessibleTable::selectedItems()` 始终返回 `selectedCells()`；视频表为 3 列时，`SelectRows` 会让当前行产生 3 个 selected index。macOS 改用 `SelectItems + SingleSelection`，把 AX 选择结果限制为当前 1 个 cell，同时保留 `currentRow()`、翻页预览和模板按钮交互；其他平台继续使用 `SelectRows`。临时冻结 harness 已用原生 picker 分别完成 15 页和 17 页真实点选，进程保持存活且 AX 状态与界面截图可读；正式冻结包仍待验收。
