...
### 1.3 模板删除
- [x] 点击「批量删除」弹出确认对话框，确认后删除对应 JSON 文件
- [x] 支持 Ctrl/Shift 多选模板后一次删除多个模板
- [x] 删除后列表自动刷新，清空当前表单
...
- [x] 选择一个主文件夹，支持两种目录结构：
  ...
  - **直接平铺**：主文件夹下直接放图片（无子文件夹），以所选文件夹名作为组名
...
- [x] 拼图页输入入口简化为「导入文件夹 / 导入文件」；文件夹入口自动识别单文件夹或批量子文件夹，文件入口支持多图和单个/多个 PPT
- [x] 拼图导出每次按来源名称分配新目录；重名时追加 `_2`、`_3`，旧成品不覆盖、不清理
- [x] 拼图右侧预览刷新做防抖和图片缓存，调整行列数时不再每一步都重新读原图并立刻重绘
- [x] 多 PPT / 多文件夹切换时保留每个来源的布局、行列、排除页、预览页和输出张数；批量导出按各来源已保存设置执行
- [x] 图片文件夹包含多个子文件夹时显示来源列表，可像多 PPT 一样逐份切换并分别保留设置
- [x] 多来源时显示「导出当前」和「导出全部」，单来源时只显示「开始导出」
- [x] 行数、列数、快捷预设、自动适配和背景色变化会直接调度右侧预览刷新
- [x] 修改布局行列或快捷预设后，自动拆分张数会按新容量重新计算，避免沿用旧布局张数导致漏页
- [x] `.rongjing_collage_manifest.json` 只记录本轮新目录内的成品，不再用于清理任何旧导出目录
- [x] 导入图片文件夹时跳过本软件已导出的拼图结果目录，避免把成品子文件夹再次当作输入来源
- [x] 同批导入同名 PPT 时，缓存目录按文件路径哈希隔离，界面来源名自动加序号，避免页图和输出目录互相覆盖
- [x] 点击「导出全部」时冻结每份来源的配置快照，运行期间继续操作界面也不会影响本轮导出队列
...
- [x] 批量导出扫描时跳过隐藏文件和 macOS `._` 元数据文件；运行时遇到无法识别的图片会跳过并继续处理
...
- [x] 支持「上大图」拼图布局：每张拼图顶部显示 1 张大图，下方按行列数显示多张小图，并按容量自动切换下一张拼图
...
## 十五、资料导出、统一文件规则与页面翻页视频（2026-09-09）
...
- [x] 所有扫描入口过滤隐藏文件、AppleDouble `._*`、Office 临时文件 `~$*` 和非文件项
- [x] 过滤早于排序、计数、封面选择、manifest 生成和任务清单组装
- [x] 根目录直接图片使用所选文件夹名作为来源名
- [x] 拼图、图片合成、真实视频和资料导出每次创建新的来源目录；同名时追加 `_2`、`_3`，旧产物不覆盖、不清理
...
- [x] 正式页面视频复用批量图片分辨率规则：`0` 为原始/模板尺寸，默认宽度 1920，可选 2560 或 3840；预览固定宽度 960，真实视频不套用
- [x] 背景缩放使用 LANCZOS，同尺寸背景不重复 resize
- [x] Mac 优先使用 VideoToolbox，码率限制为 8–20 Mbps；无法真正打开时回退 libx264 CRF 17
- [x] 翻页方向默认为从右向左，可选从左向右；正式导出与预览、CPU 与 Core Image 共用同一方向
- [x] 页面图片模式提供「预览翻页」按钮，固定使用前两张有效页面与首个屏幕模板，在 900×560 应用内弹窗循环播放；支持播放、暂停、重新播放和关闭
- [x] 预览缓存使用专用 `page_preview_cache`，唯一 MP4 平铺存放；启动时通过 no-follow `dir_fd` 只清理直属层超过 24 小时的普通 MP4，不处理目录、符号链接或其他文件
- [x] 音乐库固定存放于 App Data 的 `融景/music/`，支持导入音频、文件夹与视频；视频只提取首条音轨，不保存画面，并按 SHA-256 去重
- [x] 页面视频支持不配乐、固定配乐、随机配乐，默认音量 35%；从音频第 0 秒开始，短音频每轮也从第 0 秒循环同一首曲目，输出 48 kHz 双声道 AAC
- [x] 随机模式在完成回执中报告实际曲目；真实视频原声链路保持不变
- [x] 翻页预览视频包含所选 BGM，当前应用内预览弹窗仅显示画面，不播放声音
...
- [x] 音乐库与页面视频配乐由 M1 `7d3ebc2`、M2 `00103fe`、M3 `96ccba6` 实现；独立审查结论为 PASS WITH RISKS
- [x] 页面视频清晰度与 BGM 零秒起播修复 `9a6889d`：短样本回读 1920×1440 / 1024×768，编码前后 ROI 边缘能量 9.710 / 9.887，前 200 ms RMS 0.0963，与源开头相关系数 0.9998
- [ ] 音乐库与配乐的真机听感、长音频和冻结 App 未验证；AAC 尾垫与未持久 sidecar 为非阻断风险
